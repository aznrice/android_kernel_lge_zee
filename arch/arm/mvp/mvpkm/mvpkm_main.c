 ...
 * @param forRegion Annotation for Page pool to identify guest page allocations
 * @return INVALID_MPN: no host memory available<br>
                     ...
                     HKVA *hkvaRet)
{
    ...
    * page_to_pfn should NEVER return our INVALID_MPN magic number here
    * (or greater). Else this will be a big mess... (LPAE support?)
    */
      mpn = page_to_pfn(page);

    /*
     * System RAM bank in 0x00000000 workaround. Should only happens once
     * in host lifetime. See MVP-4855.
     */
   } while (mpn == 0);
