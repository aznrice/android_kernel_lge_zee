...
QUEUE_BUFF_ERROR2:
	...
	ion_free(cpp_dev->client, buff->map_info.ion_handle);     /* LGE_CHANGE, move a ion_free function from QUEUE_BUFF_ERROR1 to QUEUE_BUFF_ERROR2 to avoid a kernel crash - QCT patch CN#01344542:, 2013-12-6, jonghwan.ko@lge.com */
QUEUE_BUFF_ERROR1:
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_CPP_GET_INST_INFO: {
		...
		memset(&inst_info, 0, sizeof(struct msm_cpp_frame_info_t));  /* LGE_CHANGE, Set inst_info before send it to userspace, 2013-12-06, jungki.kim@lge.com */
		for (i = 0; i < MAX_ACTIVE_CPP_INSTANCE; i++) {
