...
wl_android_set_miracast(struct net_device *dev, char *command, int total_len)
{
	...
	case MIRACAST_MODE_SOURCE:
		...
		//ret = wldev_iovar_getint(dev, "bi", &val);
		ret = wldev_ioctl(dev, WLC_GET_BCNPRD, &val, sizeof(int), false);//20140206,changed for BCM4335
		if(!ret && val > 100)
		{
			config.param = 0;
			DHD_ERROR(("%s: Connected wlan0's beacon interval: %d and set mchan_algo to %d \n",__FUNCTION__, val, config.param));
		}
		else
#endif
