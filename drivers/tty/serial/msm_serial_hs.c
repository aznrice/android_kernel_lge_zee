...
{
/* QCT Patch on unbalanced_clock_vote (based AU161) */
	int rc = atomic_read(&msm_uport->clk_count);
	//[Apply QCT Patch CASE:01316056] Kernel Crash in UART closing
	//if (rc <= 0) {
	if (rc < 0) {
...
static void msm_hs_stop_rx_locked(struct uart_port *uport)
{
	...
	//[Apply QCT Patch CASE:01316056] Kernel Crash in UART closing  [s]	
	if(MSM_HS_CLK_OFF == msm_uport->clk_state)
		return;
	//[Apply QCT Patch CASE:01316056] Kernel Crash in UART closing  [e]	

	//remove by QCT recommand [Apply QCT Patch CASE:01316056]
	//msm_hs_clock_vote(msm_uport);
	/* disable dlink */
	data = msm_hs_read(uport, UARTDM_DMEN_ADDR);
	if (is_blsp_uart(msm_uport))
