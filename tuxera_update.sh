...
build_package() {
    ...
    if [ -L "${OUTPUT_LINK}" ] ; then
        SEARCHPATHS="${SEARCHPATHS} $(for d in $archdirs; do echo "${OUTPUT_LINK}/arch/$d"; done) ${OUTPUT_LINK}/include ${OUTPUT_LINK}/scripts"
    fi
...
#
# Upload kernel headers package using SFTP.
# First downloads an SSH key for passwordless authentication.
#
upload_package_sftp() {
    ...
    # We can't prompt the user for fingerprint here - have to use StrictHostKeyChecking=no.
    # Fix the file permissions of the uploaded package.
...
check_autobuild_prerequisites() {
    ...
    # sftp is needed due to wget limitations
    if [ "$http_client" = "wget" ] ; then
